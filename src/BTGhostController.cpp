...
BTPacmanController::BTPacmanController(std::shared_ptr<Character> character):Controller(character),root(std::make_shared<Selector>())  {
	...
	//Rama3 default, comer la másc cercana
...
// GhostNearby: Detecta si hay un fantasma No comestible cerca a menos de 5 casillas, si es comestible, no es peligroso
...
//PowerPillNearby: Detecta si hay una powerpill cerca a menos de 10 casillas, va a buscarla solo si hay fantasmas comestibles cerca
...
//PacmanFlee: Huir del fantasma más cercano, eligiendo la casilla vecina más lejos de él
Status PacmanFlee::update() {
    auto gs        = Info::getInfo()->in_gamestate;
    auto character = Info::getInfo()->in_character;
    auto pacmanPos = gs->getMaze().getNodePos(gs->getPacmanPos());
 
    // Encontrar el fantasma peligroso más cercano
    ...
    // Moverse a la casilla más lejos de ese fantasma
...
// GoToPowerPill: Ir hacia la power pill más cercana
Status GoToPowerPill::update() {
    ...
    // Encontrar la power pill más cercana
...
// EatPills: ir hacia la píldora normal más cercana
Status EatPills::update() {
    ...
    // Encontrar la píldora más cercana
